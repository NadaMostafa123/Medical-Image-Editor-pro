...
class ZoomDialog(QDialog):
    ...
    def apply_zoom(self):
        ...
        if new_width > 0 and new_height > 0:
            ...
            # Update the original image with the zoomed image
...
class DicomImageEditor(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("DICOM Image Editor")
    ...
    def setup_ui(self):
        ...
        self.save_button1.clicked.connect(lambda: self.save_image_as_standard(self.edited_image_label1))
        self.sidebar_layout.addWidget(self.save_button1)

        self.save_button2 = QPushButton("Save Image 2")
        self.save_button2.clicked.connect(lambda: self.save_image_as_standard(self.edited_image_label2))
    ...
    def save_image_as_standard(self, edited_image_label):
        """Save the edited image as a standard image file."""
        ...
        edited_image_label.pixmap().save(file_path)

        QMessageBox.information(self, "Success", f"Image saved as standard file:\n{file_path}")
    ...
    def apply_zoom(self):
        ...
        if new_width > 0 and new_height > 0:
            ...
            # Update the original image with the zoomed image
            self.original_pixel_array = self.current_image.copy()
...
"""
    ...
    def calculate_cnr(self):
        ...
        try:
            ...
            cnr = abs(signal_mean - second_mean) / reference_mean if reference_mean != 0 else 0
    ...
    def clear_results(self):
        ...
        self.selected_reference = None
        self.selected_2nd = None
        self.edited_image_label1.clear()  # Clear the label instead of setting to None
        self.edited_image_label2.clear()  # Clear the label instead of setting to None
